...
export function TradeReviewForm({ jobId, revieweeName, jobType }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await submitReview({
        ...
        scope_clarity_score: scopeScore || undefined,
    ...
    <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8">
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <RatingField label="Payment" value={paymentScore} onChange={setPaymentScore} disabled={isPending} />
          <RatingField label="Communication" value={communicationScore} onChange={setCommunicationScore} disabled={isPending} />
          <RatingField label="Scope clarity" value={scopeScore} onChange={setScopeScore} disabled={isPending} />
          <RatingField label="Site access" value={siteAccessScore} onChange={setSiteAccessScore} disabled={isPending} />
        </div>

        {/* Would work again */}
        <div>
          <p className="text-sm font-medium text-gray-700 mb-2">Would work with again?</p>
          <div className="flex gap-3">
            {([true, false] as const).map(val => (
              <button
                key={String(val)}
                type="button"
                onClick={() => setWouldWorkAgain(wouldWorkAgain === val ? null : val)}
                disabled={isPending}
        ...
        <div>
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-gray-700">Raise a red flag?</p>
            <button
              type="button"
              onClick={() => { setRedFlag(!redFlag); setRedFlagReason('') }}
              disabled={isPending}
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors disabled:opacity-50
                ${redFlag ? 'bg-red-500' : 'bg-gray-200'}`}
              role="switch"
              aria-checked={redFlag}
            >
              <span className={`inline-block h-4 w-4 rounded-full bg-white transition-transform
                ${redFlag ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
          <p className="mt-1 text-xs text-gray-400">
            Red flags are visible on the client&apos;s profile and affect their score.
          </p>
          {redFlag && (
            <div className="mt-3">
              <select
                value={redFlagReason}
                onChange={e => setRedFlagReason(e.target.value as RedFlagReason)}
                disabled={isPending}
                className={selectCls(!!fieldErrors.red_flag_reason)}
              >
              ...
              {fieldErrors.red_flag_reason && (
                <p className="mt-1 text-xs text-red-600">{fieldErrors.red_flag_reason}</p>
              )}
            </div>
          )}
        </div>
